...
export function ReplyForm({
  ...
  function onSubmit(formData: FormData) {
    startTransition(async () => {
      const res = await replyAction(slug, feedbackId, formData);
      if (res.ok) {
    ...
    <form ref={formRef} action={onSubmit} className="space-y-3">
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Input id="authorName" name="authorName" required autoComplete="name" defaultValue={defaultName} />
        </div>
        <div className="space-y-1.5">
          ...
          <Input id="authorEmail" name="authorEmail" type="email" autoComplete="email" defaultValue={defaultEmail} />
        </div>
      </div>
      <div className="space-y-1.5">
        <label htmlFor="body" className="text-xs font-medium text-muted-foreground">Reply</label>
